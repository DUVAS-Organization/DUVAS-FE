...
const navLinks = [
    { name: "Trang Chủ", path: "/" },
    // "Phòng trọ - Căn hộ" sẽ được render riêng dưới dạng droplist
...
const Navbar = () => {
    ...
    const [roomDropdownOpen, setRoomDropdownOpen] = useState(false); // Dropdown "Phòng trọ - Căn hộ"
    const dropdownRef = useRef(null);
    const location = useLocation(); // ADDED
    const queryParams = new URLSearchParams(location.search);
    const tab = queryParams.get("tab") || "";
    const isActiveDropdown = tab === "Phòng trọ" || tab === "Căn hộ" || tab === "Nhà nguyên căn"; // ADDED
    // Hook useEffect: đóng dropdown user khi click ngoài
    useEffect(() => {
            ...
            <div className="max-w-full mx-auto px-2 sm:px-4 lg:px-8">
                <div className="relative flex items-center h-16">
                    ...
                    <div className="hidden sm:flex space-x-1">
                        ...
                        {/* Nút "Phòng trọ - Căn hộ" với droplist (click để mở) */}
                        <RoomDropdown />

                        {/* Các mục còn lại (Tin Dịch vụ, Thông tin) */}
